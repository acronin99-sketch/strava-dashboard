...
export default async function Home({ searchParams }: PageProps<"/">) {
  ...
  const summary = totals(activities);
  const weekly = weeklyTotals(activities, 12);
  const sports = bySport(activities);
  const runPace = paceTrend(activities, "Run", 30);
    ...
    <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-10">
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <Card
          ...
          subtitle={
            runPace.length
              ? "Last 30 runs · higher is faster"
              : "No runs in this range"
          }
        >
          {runPace.length ? (
            <PaceChart data={runPace} />
          ) : (
            ...
            <table className="w-full text-left text-sm">
              <thead className="text-xs uppercase tracking-wide text-zinc-500">
                <tr>
                  ...
                  <th className="px-1 pb-2 text-right font-medium">Pace</th>
